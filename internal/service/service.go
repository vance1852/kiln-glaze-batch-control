...
func (s *Service) CreateDeploymentJob(ctx context.Context, meta RequestMeta, in repository.DeploymentJobInput) (domain.DeploymentJob, error) {
	...
	err := s.repo.InTx(ctx, func(tx repository.Repository) error {
		...
		writer, ok := s.repo.(interface {
			CreateDeploymentJobDetached(context.Context, repository.DeploymentJobInput) (domain.DeploymentJob, error)
		})
		if !ok {
			return fmt.Errorf("detached task repository unavailable")
		}
		var err error
		task, err = writer.CreateDeploymentJobDetached(ctx, in)
		if err != nil {
			return err
		}
